...
export default function ProfilePage() {
  ...
  const handleVerifyCodeChange = (index: number, value: string) => {
    ...
    setVerifyCode(next);
    if (value && index < 5) document.getElementById(`email-verify-${index + 1}`)?.focus();
  };
            ...
            <div className="flex gap-2 justify-center mb-4">
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Backspace' && !verifyCode[i] && i > 0 && document.getElementById(`email-verify-${i - 1}`)?.focus()}
